...
- [ ] **Task 34:** Add API versioning prefix to all backend endpoints.
  - **Design Goal:** Enable safe API evolution without breaking existing integrations. Currently, routes are registered directly as `/api/parse`, `/api/fuzz/start`, etc. in [main.go](file:///Users/alex/src/swazz/packages/container/main.go) (L340-352) using Gin router groups.
  - **Implementation Details:**
    - **Backend:** In [main.go](file:///Users/alex/src/swazz/packages/container/main.go), change the route group from `r.Group("/api")` to `r.Group("/v1/api")`. Add backward-compatible redirects:
      ```go
      // Legacy unversioned routes → redirect to /v1/
      legacy := r.Group("/api")
      legacy.Any("/*path", func(c *gin.Context) {
          c.Redirect(http.StatusTemporaryRedirect, "/v1"+c.Request.URL.Path)
      })
      ```
    - **Vite proxy:** In [vite.config.ts](file:///Users/alex/src/swazz/packages/web/vite.config.ts), update proxy:
      ```ts
      proxy: {
          '/v1/api': { target: 'http://localhost:8080', changeOrigin: true },
          '/api': { target: 'http://localhost:8080', changeOrigin: true },  // legacy fallback
      }
      ```
    - **Frontend fetch URLs:** Update all 9 fetch call sites:
      - [swaggerService.ts](file:///Users/alex/src/swazz/packages/web/src/services/swaggerService.ts) L9: `/api/parse` → `/v1/api/parse`
      - [useRunner.ts](file:///Users/alex/src/swazz/packages/web/src/hooks/useRunner.ts) L119, L146, L163, L204, L214, L220: all `/api/fuzz/*` → `/v1/api/fuzz/*`
      - [useRunHistory.ts](file:///Users/alex/src/swazz/packages/web/src/hooks/useRunHistory.ts) L78: `/api/report` → `/v1/api/report`
      - [PayloadSettingsModal.tsx](file:///Users/alex/src/swazz/packages/web/src/components/Sidebar/PayloadSettingsModal.tsx) L20: `/api/payload-catalog` → `/v1/api/payload-catalog`
    - **Health check:** Keep `/health` unversioned (it's infrastructure, not API).
    - **Tests:** Verify both `/v1/api/parse` (200) and legacy `/api/parse` (307 redirect) work.
