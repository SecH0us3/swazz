...
func ToSSE(r *swagger.FuzzResult) *swagger.FuzzResultSSE {
	...
	// Resolve OWASPCategory, OWASPAPICategory, and CWEIDs for a safe copy of
	// AnalyzerFindings, keeping whatever taxonomy the analyzer already supplied.
	...
	if r.AnalyzerFindings != nil {
		...
		for i, f := range r.AnalyzerFindings {
			f.OWASPCategory, f.OWASPAPICategory, f.CWEIDs = classifier.ResolveTaxonomy(&f, r.Method, r.Endpoint)
...
func truncateValue(v any, maxLen int) any {
	...
	// Fast path for simple primitive types to avoid JSON round-trip
	switch val := v.(type) {
	case string:
		if len(val) > maxLen {
	...
	// Always normalize maps, slices, and structs to generic JSON map/slice structure
	// to avoid concrete nested types (like []string or map[string]string) bypassing recursive checks.
