...
func TestHarvester_FullLifecycle(t *testing.T) {
	...
	r.harvestFromResponse("/api/v1/organizations/{orgId}/users", "GET", 403, map[string]any{"id": "ignored"})
	assert.Empty(t, r.collectAllHarvestedIDs())
	...
	r.harvestFromResponse("/api/v1/organizations/{orgId}/users", "GET", 200, resp1)
	...
	r.harvestFromResponse("/api/v1/projects", "GET", 200, resp2)

	allIDs := r.collectAllHarvestedIDs()
	require.ElementsMatch(t, []string{"usr-001", "usr-002", "proj-999"}, allIDs)

	// Check idSources
	assert.Equal(t, "GET /api/v1/organizations/{orgId}/users", r.idSourceFor("usr-001"))
	assert.Equal(t, "GET /api/v1/projects", r.idSourceFor("proj-999"))
}
