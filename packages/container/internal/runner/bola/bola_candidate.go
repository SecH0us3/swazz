...
func (d *Detector) identifyCandidates(results []*swagger.FuzzResult) ([]*swagger.FuzzResult, map[string]bool) {
...
func (d *Detector) generateMissingCandidates(ctx context.Context, hasSuccessCandidate map[string]bool) []*swagger.FuzzResult {
	var (
	...
	d.ctx.AddTotalEndpoints(numMissingEndpoints)
	...
	for _, ep := range d.ctx.Config().Endpoints {
		...
		go func(ep swagger.EndpointConfig) {
			...
			d.ctx.UpdateProgressEndpoint(ep.Method + " " + ep.Path)
			d.ctx.BroadcastProgress()

			epGen := generator.New(d.ctx.Config().Dictionaries, swagger.ProfileRandom, d.ctx.Config().Settings)
			epGen.RunID = d.ctx.Config().RunID
			if res := d.generateCandidateForEndpoint(ctx, ep, epGen); res != nil {
				candMu.Lock()
				candidates = append(candidates, res)
				candMu.Unlock()
			}
...
func (d *Detector) generateCandidateForEndpoint(
	ctx context.Context,
	...
	for i := range limit {
		...
		headers := d.globalHeadersWithGenerated(safeGen.GenerateSecurityHeaders())
		d.ctx.AddTotalPlanned(1)

		res := d.ctx.ExecuteRequest(
			ctx,
			d.ctx.Config().BaseURL, resolvedPath, ep.Path, ep.Method,
			headers, d.ctx.Config().Cookies,
			payload,
