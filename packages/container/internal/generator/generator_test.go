...
func TestToXML(t *testing.T) {
	// 1. Flat structure with escaping
	flatData := map[string]any{
		"name": "john & doe",
		"age":  30,
	}
	flatXml, err := ToXML(flatData, "person")
	if err != nil {
		t.Fatalf("ToXML failed: %v", err)
	}
	expectedFlat := "<person><age>30</age><name>john &amp; doe</name></person>"
	if flatXml != expectedFlat {
		t.Errorf("Expected flat XML %q, got %q", expectedFlat, flatXml)
	}

	// 2. Single-key map (mimicking SOAP)
	soapData := map[string]any{
		"TradePriceRequest|http://example.com/stockquote.xsd": map[string]any{
			"tickerSymbol": "MSFT",
		},
	}
	soapXml, err := ToXML(soapData, "request")
	if err != nil {
		t.Fatalf("ToXML failed: %v", err)
	}
	expectedSoap := "<TradePriceRequest xmlns=\"http://example.com/stockquote.xsd\"><tickerSymbol>MSFT</tickerSymbol></TradePriceRequest>"
	if soapXml != expectedSoap {
		t.Errorf("Expected SOAP XML %q, got %q", expectedSoap, soapXml)
	}

	// 3. Nested array
	nestedArray := map[string]any{
		"TradePriceRequest": map[string]any{
			"tickers": []any{"MSFT", "AAPL"},
		},
	}
	arrayXml, err := ToXML(nestedArray, "request")
	if err != nil {
		t.Fatalf("ToXML failed: %v", err)
	}
	expectedArray := "<TradePriceRequest><tickers>MSFT</tickers><tickers>AAPL</tickers></TradePriceRequest>"
	if arrayXml != expectedArray {
		t.Errorf("Expected array XML %q, got %q", expectedArray, arrayXml)
	}
}
