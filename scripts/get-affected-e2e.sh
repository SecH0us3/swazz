...
if [ -z "$MERGE_BASE" ]; then
  # If no merge base found (shallow clone or initial commit), run smoke test
  echo "tests/e2e/swazz.spec.ts"
  exit 0
fi
...
while IFS= read -r file; do
  ...
  case "$file" in
    # Global / Infrastructure changes that alter E2E runtime → trigger full suite
    playwright.config.ts|package.json|package-lock.json|tests/e2e/run-e2e.sh|packages/edge/wrangler.toml)
      RUN_ALL=true
      break
      ;;
    ...
    # Generic frontend/edge files
    packages/web/*|packages/edge/*)
      HAS_UI_CHANGES=true
      RUN_ALL=true
      break
      ;;

    # Pure backend container engine, demo, scripts, docs, CI workflows
    packages/container/*|demo/*|scripts/*|*.md|docs/*|.github/*)
      ;;

    # Any other unmapped file
    *)
      ;;
  esac
done <<< "$CHANGED_FILES"
...
if [ "$HAS_UI_CHANGES" = false ] && [ ${#SPECS[@]} -eq 0 ]; then
  echo "NONE"
  exit 0
fi
